...
COBBLESTONE_ROAD_CFG = terrain_gen.TerrainGeneratorCfg(
    ...
    use_cache=False,
    sub_terrains={
        "flat": terrain_gen.MeshPlaneTerrainCfg(proportion=1.0), # Use only flat terrain for standing
    },
)
...
class EventCfg:
    ...
    base_external_force_torque = EventTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names="base"),
            "force_range": (0.0, 0.0), # No external force for standing task stability
    ...
    reset_robot_joints = EventTerm(
        ...
        params={
            ...
            "velocity_range": (0.0, 0.0), # Reset with zero joint velocity
        },
    )
...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformLevelVelocityCommandCfg(
        ...
        rel_standing_envs=1.0, # mostly standing
        debug_vis=True,
        ranges=mdp.UniformLevelVelocityCommandCfg.Ranges(
            lin_vel_x=(0.0, 0.0), lin_vel_y=(0.0, 0.0), ang_vel_z=(0.0, 0.0) # Zero velocity command
        ),
...
class RewardsCfg:
    ...
    # -- task
    # We want zero velocity, so we reward staying at zero or penalize deviation
    # Ideally track_lin_vel_xy is good if command is 0.
    ...
    joint_vel = RewTerm(func=mdp.joint_vel_l2, weight=-0.005) # Increased penalty for stillness
    joint_acc = RewTerm(func=mdp.joint_acc_l2, weight=-2.5e-7)
    joint_torques = RewTerm(func=mdp.joint_torques_l2, weight=-2e-4)
    action_rate = RewTerm(func=mdp.action_rate_l2, weight=-0.1)
    dof_pos_limits = RewTerm(func=mdp.joint_pos_limits, weight=-10.0)
    energy = RewTerm(func=mdp.energy, weight=-1e-4) # Increased penalty for efficiency

    # -- robot
    # Heavy penalty for bad orientation
    ...
    # -- feet
    # Reward for keeping feet on ground
...
class CurriculumCfg:
    ...
    # No curriculum needed for static standing
    pass
